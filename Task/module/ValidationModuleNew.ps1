...
if($match -eq 0)
{
    Write-Host "##[error] None of the following builds, $($definitions -replace ";", ", "), were not found in the repository '$($repositoryName)'";
    Write-Output "##vso[task.complete result=Failed;]DONE"
    #Exit 1;
}
...
# ********************************** #
#                                    #
# MAIL SEND OPTION SELECTED IF ERROR #
#                                    #
# ********************************** #
if([bool]$MailSend -and ($buildFailed -eq "Error" -or $buildStatus -eq "inProgress"))
{
...
#FINALISE TASK
    ...
    if ($buildFailed -eq "Failed")
    {
        Write-Host "##[error] Builds for $($ReleaseVersion) have failed."
        Write-Output "##vso[task.complete result=Failed;]DONE"
    }
    elseif ($buildStatus -eq "inProgress")
    {
